...
Five worlds, picked from **Style** in the status item menu or **View → Style** in the menu
...
| Iceberg Floe | Solid sea under a wall of ice cliffs, a handful of drifting floes | A pixel penguin, a different look for each agent |

All five keep the same daylight cycle and read the same activity; only the inhabitants change,
so a cat, a rabbit, a witch or a penguin sits, hops, flies or waddles about and curls up asleep
wherever a person would have stood, strolled and lain down. Switching styles repaints the plaza
and everyone walks back in.
...
One pixel person — or one cat, or one rabbit, or one witch, or one penguin — per client *and agent*, not per conversation: a Mac running
